 ...
 */
export function parseMeleeData(json: unknown): MeleeData {
  return json as MeleeData;
}
 ...
 */
export function getMeleeHistoryForGrower(data: MeleeData, growerName: string): MeleeAppearance[] {
  const results: MeleeAppearance[] = [];

  for (const yearData of data.years) {
    let deepest: MeleeRound | null = null;

    for (const side of [yearData.bracket.left, yearData.bracket.right] as MeleeSide[]) {
      for (const region of [side.topRegion, side.bottomRegion] as MeleeRegion[]) {
        // Round of 16
    ...
    // Championship
    if (
      nameMatch(yearData.bracket.final.p1, growerName) ||
